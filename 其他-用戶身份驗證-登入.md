...
## 摘要
登入的行為主要分為三個步驟, 主要目的是為了產生 Session並且儲存於用戶的 Cookie, 而後端會暫存 Session在 Redis. Session則是會用來讓其他 HTTP API在呼叫時使用, 其他 API呼叫時藉由 Session就能夠判斷用戶有無登入.  
...
### 3.2.6. 紀錄帳號登入資訊
此步驟會將這一次登入的相關資料(帳戶ID、使用者階層、當下的時間、登入成功或失敗的資訊)紀錄在 db內. 此資料延伸可應用於觀察用戶登入資訊.
